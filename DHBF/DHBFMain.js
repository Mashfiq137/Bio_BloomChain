...
const { RunSimulation_DHBF, GenerateRandomSubsetIdentities, EstimateGasCosts } = require("./helper-functions/Simulation");
const { writeOutputToCSV_DHBF, writeGasEstimationOutputToCSV } = require("./helper-functions/FileIO");
...
async function DHBFMain() {
    ...
    const { gasCosts } = await RunSimulation_DHBF(contractAddress, multiInstanceMasterArray, multiInstanceQueryIdentities)
    writeGasEstimationOutputToCSV(gasCosts, "./output/DHBF_Send_Packed_Data_GasCosts.csv");
}
